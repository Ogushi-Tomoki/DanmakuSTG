		...
		if (effect[i].flag>0) {//エフェクトが登録されていたら
			...
			case 4://喰らいボムエフェクト
				...
				if (effect[i].cnt >= 12 || ch.flag != 1)
					effect[i].flag = 0;
				break;
			case 5://チャージボム爆発エフェクト
				//明るさと大きさ計算
				effect[i].ang += PI2 * 121 / 360;
				if (effect[i].cnt < 120) {
					effect[i].r += 0.001;
				}
				if (effect[i].cnt >= 120) {
				...
				break;
			case 6://爆発エフェクト
				effect[i].img = img_bakuhatsu[effect[i].cnt / 4];
				effect[i].cnt++;
				if (effect[i].cnt>24)//20カウント以上になったら
					effect[i].flag = 0;//消す
				break;
			default:
...
//爆発エフェクトを登録する
void enter_bakuhatsu_effect(int i) {
	int k;
	if ((k = search_effect()) != -1) {
		effect[k].flag = 1;
		effect[k].cnt = 0;
		effect[k].knd = 6;//爆発エフェクト
		effect[k].brt = 255;
		effect[k].ang = -PI / 2;
		effect[k].mvang = -PI / 2;
		effect[k].spd = 0;
		effect[k].r = 1;
		effect[k].eff = 1;
		effect[k].img = img_bakuhatsu[0];
		effect[k].x = cshot[i].x;
		effect[k].y = cshot[i].y;
	}
}
...
//エンペラータイム計算
void emperor_time_calc() {
	if (ch.flag == 0 && emperor_time.flag == 0 && emperor_time.num > 0) {//キャラが通常で、ボムがオフであり且つボムが残っていたら
		if (CheckStatePad(configpad.bom) == 1) {//ボムボタンが押されたら
			emperor_time.flag = 1;
			emperor_time.cnt = 0;
			emperor_time.num -= 1;
			emperor_time.spd_rate = 1.0 / (double)EMPEROR_TIME_RATE;
			se_flag[16] = 1;
		}
	}
	if (emperor_time.flag == 1) {//エンペラータイムがが登録されていたら
		emperor_time.cnt++;
		if (emperor_time.cnt == 1)
			bright_set.brt = 155;//画面の明るさ設定(暗く)
		if (emperor_time.cnt>300) {
			emperor_time.flag = 0;
			emperor_time.cnt = 0;
			emperor_time.spd_rate = 1.0;
			bright_set.brt = 255;
			se_flag[17] = 1;
		}
	}
...
//チャージボム計算
void charge_bom_calc() {
	if ((ch.flag == 0 || ch.flag == 1) && charge_bom.flag == 0) {//キャラが通常か喰らいボム状態で、ボムがオフなら
		if (ch.gauge == GAUGE_MAX && CheckStatePad(configpad.special) == 1) {//フルチャージされた状態でスペシャルボタンが押されたら
			enter_charge_bom();
			//enter_bom();
			ch.gauge = 0;
			ch.gauge_flag = 0;
	...
	if (charge_bom.flag == 1) {//ボムが登録されていたら
		charge_bom.cnt++;
		if (charge_bom.cnt < 120) {
			charge_bom.r = 256 * (0.01 + 0.001*charge_bom.cnt);
		}
		if (charge_bom.cnt >= 120) {
			charge_bom.r = 256 * (0.01 + 0.001 * 120 + 0.03*(charge_bom.cnt - 120));
		}
		//if (charge_bom.cnt<40)
		//	bright_set.brt = 255 - charge_bom.cnt * 5;//画面の明るさ設定(暗く)
		//if (charge_bom.cnt>200)
		//	bright_set.brt = 255 - 40 * 5 + (charge_bom.cnt - 200) * 5;//画面の明るさ設定(明るく)
		if (charge_bom.cnt>200) {
			charge_bom.flag = 0;
			charge_bom.cnt = 0;
			charge_bom.r = 0;
...
void effect_main() {
	...
	emperor_time_calc();//ボム計算
	charge_bom_calc();//チャージボム計算
	enter_crybom_effect();
}