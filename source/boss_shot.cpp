...
double bossatan4(double x1, double y1, double x2, double y2) {//自機と指定座標のなす角
...
void boss_shot_calc() {
	int i;
	if (emperor_time.flag == 0 || count % EMPEROR_TIME_RATE == 0) {		//エンペラータイムが発動していなければ
		boss.endtime--;
	}
	if (boss.endtime<0)
		boss.hp = 0;
	for (i = 0; i<BOSS_BULLET_MAX; i++) {
		if (boss_shot.bullet[i].flag>0) {
			if (emperor_time.flag == 0) {		//エンペラータイムが発動していなければ
				...
				boss_shot.bullet[i].x += cos(boss_shot.bullet[i].angle)*boss_shot.bullet[i].spd*emperor_time.spd_rate;
				boss_shot.bullet[i].y += sin(boss_shot.bullet[i].angle)*boss_shot.bullet[i].spd*emperor_time.spd_rate;
				if (count % EMPEROR_TIME_RATE == 0) {
					boss_shot.bullet[i].cnt++;
				}
			}
...
void calc_boss() {
	if (emperor_time.flag == 0) {		//エンペラータイムが発動していなければ
		...
		boss.dx = boss.x;
		boss.dy = boss.y + sin(PI2 / 130 * (count % 130)) * 10 * emperor_time.spd_rate;
	}
}

extern void enter_boss_item();
...
void boss_shot_main() {
	...
	if (boss.state == 2 && (boss.hp <= 0 || boss.endtime <= 0)) {//弾幕中で体力が無くなったら
		...
		if (ch.gauge < GAUGE_MAX && ch.gauge_flag == 1) {	//ゲージを増やす
			ch.gauge += (emperor_time.flag == 0 ? 20.0 : 30.0);
