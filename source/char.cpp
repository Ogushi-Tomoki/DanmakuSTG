...
void calc_ch() {
	if (ch.flag == 1) {//喰らいボム受付中
		...
		if (ch.cnt > (ch.gauge == GAUGE_MAX ? 30 : 10)) {//0.66秒受け付ける
	...
	if (ch.cnt == 0 && ch.flag == 2) {//今の瞬間死んだら
		...
		if (emperor_time.flag == 1) {		//エンペラータイム発動中なら
			emperor_time.flag = 0;
			emperor_time.cnt = 0;
			emperor_time.spd_rate = 1.0;
			se_flag[17] = 1;
		}
		if (ch.num < 0) {		//残機が無くなったら
	...
	if (charge_bom.flag == 1) {
		ch.mutekicnt++;
		if (ch.mutekicnt > 200 && charge_bom.flag == 0) {
			ch.mutekicnt = 0;
		}
	}
	else if (ch.mutekicnt>0) {//無敵カウントが0じゃなければ
		ch.mutekicnt++;
...
void ch_move() {
	...
	double speed = 4.5;
	double move_x[4] = { -speed, speed, 0, 0 }, move_y[4] = { 0, 0, speed, -speed };	//｛左, 右, 下, 上｝のスピード
	int inputpad[4];
