	...
	for (i = 0; i<SHOT_BULLET_MAX; i++) {//n番目の弾幕データの弾を計算
		if (shot[n].bullet[i].flag>0) {//その弾が登録されていたら
			if (emperor_time.flag == 0) {		//エンペラータイムが発動していなければ
				...
				shot[n].bullet[i].x += cos(shot[n].bullet[i].angle)*shot[n].bullet[i].spd*emperor_time.spd_rate;
				shot[n].bullet[i].y += sin(shot[n].bullet[i].angle)*shot[n].bullet[i].spd*emperor_time.spd_rate;
...
void shot_main() {
	...
	for (i = 0; i<SHOT_MAX; i++) {//弾幕データ計算
		...
		if (shot[i].flag != 0 && 0 <= shot[i].knd && shot[i].knd<SHOT_KND_MAX) {
			...
			if (emperor_time.flag == 0 || count % EMPEROR_TIME_RATE == 0) {		//エンペラータイムでなければ
				shot[i].cnt++;
			}
		}
	}
