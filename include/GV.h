...
GLOBAL int img_bakuhatsu[6];//爆発の画像
...
//フラグ・ステータス変数
GLOBAL int func_state, stage_count, count, stage, setting_menu_count;	//関数制御用変数
...
GLOBAL pad_t pad;		//パッドの入力状態を格納する変数
...
GLOBAL emperor_time_t emperor_time;	//エンペラータイム（ボム）
GLOBAL charge_bom_t charge_bom;		//チャージボム
GLOBAL child_t child[CHILD_MAX];//弾幕の発射位置に表示するエフェクト
GLOBAL stage_title_t stage_title;//ステージタイトル
GLOBAL clear_t clear;//クリア表示
GLOBAL item_t item[ITEM_MAX];//アイテム
GLOBAL flash_t flash;//(42)
GLOBAL area_t area;//そのエリアで持っているデータ(48)
GLOBAL option_bb_t option_bb[4];//オプション発射元ボンボン(49)
GLOBAL save_data_t save_data;
